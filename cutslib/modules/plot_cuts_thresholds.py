...
class Module:
    def __init__(self, config):
        ...
        self.gain_adj = config.getfloat("gain_adj", 5)
        self.corr_l = config.getfloat("corr_l", 0.8)
        self.rms_adj = config.getfloat("rms_adj", 5)
        self.norm_adj = config.getfloat("norm_adj", 10)
        self.de_adj = config.getfloat("de_adj", 20)
        self.mfe_adj = config.getfloat("mfe_adj", 20)
        self.jump_adj = config.getfloat("jump_adj", 3)
        self.show_presel = config.getboolean("show_presel", True)

    def run(self, proj):
        calibrate = self.calibrate
        gain_adj = self.gain_adj
        corr_l = self.corr_l
        rms_adj = self.rms_adj
        norm_adj = self.norm_adj
        de_adj = self.de_adj
        mfe_adj = self.mfe_adj
        jump_adj = self.jump_adj
        show_presel = self.show_presel
